...
const isProgramChair = (req, res, next) => {
  ...
  if (req.session.isProgramChairLoggedIn) {
    ...
    return res.status(401).json({ error: "Unauthorized ppp" });
  }
};
...
const isUserApproved = (req, res, next) => {
  if (req.session.user && req.session.user.status === 'approved') {
    ...
    return res.status(403).json({ error: "Account not approved. Please contact the program chair." });
  }
};
...
router.post('/signup', async (req, res) => {
  try {
    ...
    if (!Object.values(ROLES).map(r => r.name).includes(role) || role === 'coordinator' || role === 'programChair') {
    ...
    const newUser = new User({ userId, name, email, password: hashedPassword, role, status: 'pending' });
    const savedUser = await newUser.save();

    res.json({ message: 'User created successfully, pending approval', data: savedUser });
  } catch (error) {
...
router.post("/login", async (req, res) => {
  try {
    ...
    if (email === "programchair@iit.du.ac.bd" && password === "programchairPassword") {
    ...
    if (user.role === 'coordinator') {
      ...
      return res.json({ message: "Coordinator logged in successfully" });
    }
...
router.get('/routine-committees/:coordinatorId',isProgramChair, async (req, res) => {
  try {
...
router.put('/routine-committees/:coordinatorId',isProgramChair, async (req, res) => {
  try {
    const coordinatorId = req.params.coordinatorId;
...
router.delete('/routine-committees/:coordinatorId', isProgramChair,async (req, res) => {
  try {
    const coordinatorId = req.params.coordinatorId;
