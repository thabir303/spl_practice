...
router.post("/", async (req, res) => {
  try {
    const { courseId, courseName, credit, type, semesterName, isActive } =
      req.body;
...
// Route to delete a course by _id
...
router.delete("/:courseId/unassign", async (req, res) => {
  try {
    const { courseId } = req.params;
    ...
    if (!teacherId || !semesterName) {
      return res
        .status(400)
        .json({ error: "teacherId and semesterName are required" });
    }
    ...
    const assignmentIndex = course.assignments.findIndex(
      (a) => a.teacherId === teacherId && a.semesterName === semesterName
    );
    ...
    res
      .status(500)
      .json({ error: "Failed to unassign course", details: error.message });
  }
});
...
// Route to assign multiple courses to a teacher
router.post("/:teacherId/assign", async (req, res) => {
  try {
    const { teacherId } = req.params;
    ...
    for (const courseData of coursesData) {
      ...
      if (
        ...
        return res
          .status(400)
          .json({ error: "Missing required fields in course data" });
      }
      ...
      if (!course) {
        ...
        // Update the assignments if the course already exists
      ...
      assignedCourses.push(savedCourse);
    }
